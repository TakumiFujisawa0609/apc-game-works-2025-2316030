...
void SlotBase::Draw(void)
{
	...
	for (int i = 0; i < numSlots; ++i)
	{
		...
		// --- 1-1. 背景ボックスの描画 ---
		// 半透明の暗い背景
		// 【修正点 A】縦横のサイズを SLOT_WIDTH_RATIO / SLOT_HEIGHT_RATIO に縮小
		...
		// --- 1-2. アイコンの描画 ---
		// ... (iconColor の設定は省略せず残す)
		int iconColor = GetColor(255, 255, 255);
		if (std::dynamic_pointer_cast<Battery>(item)) iconColor = GetColor(255, 255, 255);
		else if (std::dynamic_pointer_cast<HandLight>(item)) iconColor = GetColor(255, 255, 255);
		else if (std::dynamic_pointer_cast<Hemostatic>(item)) iconColor = GetColor(255, 255, 255);
	...
	if (currentItem)
	{
		...
		int textDrawY = static_cast<int>(startY + size.height_ * (SLOT_HEIGHT_RATIO + 0.01f)); // スロットの下に配置
