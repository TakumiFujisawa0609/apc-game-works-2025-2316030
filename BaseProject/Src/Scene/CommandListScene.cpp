...
CommandListScene::CommandListScene(SceneController& controller)
	:
	Scene(controller)
{
}
...
void CommandListScene::Draw()
{
	...
	DrawBoxAA(static_cast<float>(wsize.width_ * 0.03125f), static_cast<float>(wsize.height_ * 0.04166f),
		...
		0xff00ff, false, 3.0f);
	DrawString(static_cast<int>(wsize.width_ * 0.03125f + wsize.width_ * 0.015625f), static_cast<int>(wsize.height_ * 0.04166f + wsize.height_ * 0.020833f), L"コマンド表", 0x008800);
}
